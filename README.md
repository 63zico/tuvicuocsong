...
1. Google AdSense에서 사이트 `https://tu-vi-cuoc-song.vercel.app`를 추가하고 심사를 요청합니다.
2. 승인 후 `광고 > 광고 단위 기준 > 디스플레이 광고`에서 광고 단위를 만듭니다.
3. 게시자 ID `ca-pub-...`와 광고 단위의 `data-ad-slot` 값을 복사합니다.
4. 위 전역 설정값에 실제 ID를 넣고 배포합니다.
5. 승인 직후에는 광고가 바로 안 보이거나 빈 슬롯이 나올 수 있습니다. Google이 사이트와 광고 단위를 처리하는 시간이 필요합니다.
